...
#include "track/TrackLineDescriptor.h"
...
// Our line extractor
TrackBase *extractor1;
LineTrackBase *extractor2;
...
int main(int argc, char **argv) {
  ...
  std::string config_path = "/home/zhangyanyu/catkin_ws_ov/src/open_vins/config/euroc_mav/estimator_config.yaml";
  ...
  nh->param<std::string>("path_bag", path_to_bag, "/home/zhangyanyu/Downloads/V1_01_easy.bag");
  ...
  int numlines = 100;
  parser->parse_config("num_lines", numlines, false);
  int num_pts = 400;
  int num_aruco = 1024;
  ...
  // Histogram method
  ov_core::TrackBase::HistogramMethod method1;
  ...
  if (histogram_method_str == "NONE") {
    method1 = ov_core::TrackBase::NONE;
  } else if (histogram_method_str == "HISTOGRAM") {
    method1 = ov_core::TrackBase::HISTOGRAM;
  } else if (histogram_method_str == "CLAHE") {
    method1 = ov_core::TrackBase::CLAHE;
  } else {
  ...
  ov_core::LineTrackBase::Line_HistogramMethod method2;
  parser->parse_config("line_histogram_method", histogram_method_str, false);
  if (histogram_method_str == "NONE") {
    method2 = ov_core::LineTrackBase::NONE;
  } else if (histogram_method_str == "HISTOGRAM") {
    method2 = ov_core::LineTrackBase::HISTOGRAM;
  } else {
    printf(RED "invalid feature histogram specified:\n" RESET);
    printf(RED "\t- NONE\n" RESET);
    printf(RED "\t- HISTOGRAM\n" RESET);
  ...
  // extractor1 = new TrackKLT(cameras, num_pts, num_aruco, !use_stereo, method, fast_threshold, grid_x, grid_y, min_px_dist);
  extractor1 = new TrackDescriptor(cameras, num_pts, num_aruco, !use_stereo, method1, fast_threshold, grid_x, grid_y, min_px_dist, knn_ratio);
  // extractor1 = new TrackAruco(cameras, num_aruco, !use_stereo, method, do_downsizing);
  extractor2 = new TrackLineDescriptor(cameras, numlines, num_aruco, !use_stereo, method2);
  //===================================================================================
  //===================================================================================
  //===================================================================================
...
void handle_stereo(double time0, double time1, cv::Mat img0, cv::Mat img1) {
  ...
  message.masks.push_back(mask);
  extractor1->feed_new_camera(message);
  extractor2->feed_new_camera(message);
  
  

  // Display the resulting tracks
  cv::Mat img_active, img_history;
  extractor1->display_active(img_active, 255, 0, 0, 0, 0, 255);
  extractor1->display_history(img_history, 255, 255, 0, 255, 255, 255);
  extractor2->display_active(img_active, 255, 0, 0);
  extractor2->display_history(img_history, 255, 255, 0, 255, 255, 255);
  ...
  // Get lost tracks
  std::shared_ptr<FeatureDatabase> database = extractor1->get_feature_database();
  std::vector<std::shared_ptr<Feature>> feats_lost = database->features_not_containing_newer(time0);
  num_lostfeats += feats_lost.size();
  
  std::shared_ptr<LineDatabase> linedatabase = extractor2->get_line_database();
  std::vector<std::shared_ptr<Line>> lines_lost = linedatabase->lines_not_containing_newer(time0);
  num_lostlines += lines_lost.size();

  // Mark theses feature pointers as deleted
  for (size_t i = 0; i < lines_lost.size(); i++) {
    // Total number of measurements
    int total_meas = 0;
    for (auto const &pair : lines_lost[i]->timestamps) {
      total_meas += (int)pair.second.size();
    }
    // Update stats
    lineslengths += total_meas;
    lines_lost[i]->to_delete = true;
  }
  ...
  if ((int)clonetimes.size() >= clone_states) {
    ...
    double margtime = clonetimes.at(0);
    clonetimes.pop_front();
    std::vector<std::shared_ptr<Line>> lines_marg = linedatabase->lines_containing(margtime);
    num_marglines += lines_marg.size();
    // Delete theses feature pointers
    for (size_t i = 0; i < lines_marg.size(); i++) {
      lines_marg[i]->to_delete = true;
    }
  }

  // Tell the feature database to delete old features
  linedatabase->cleanup();
  ...
  if (frames > 60) {
    // Calculate the FPS
    double fps = (double)frames / (time_curr.toSec() - time_start.toSec());
    double lpf = (double)num_lostlines / frames;
    double fpf = (double)lineslengths / num_lostlines;
    double mpf = (double)num_marglines / frames;
    // DEBUG PRINT OUT
    PRINT_DEBUG("fps = %.2f | lost_lines/frame = %.2f | track_length/lost_line = %.2f | marg_tracks/frame = %.2f\n", fps, lpf, fpf, mpf);
    // Reset variables
    frames = 0;
    time_start = time_curr;
    num_lostlines = 0;
