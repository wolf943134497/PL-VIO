...
int main(int argc, char **argv) {
  ...
  std::vector<std::vector<std::pair<size_t, Eigen::VectorXf>>> buffer_lines;
  ...
  while (sim->ok()) {
    ...
    bool hascam = sim->get_next_cam(time_cam, camids, feats, lines);
    if (hascam) {
      if (buffer_timecam != -1) {
#if ROS_AVAILABLE == 1 || ROS_AVAILABLE == 2
        viz->visualize_odometry(buffer_timecam);
#endif
        sys->feed_measurement_simulation(buffer_timecam, buffer_camids, buffer_feats, buffer_lines);
#if ROS_AVAILABLE == 1 || ROS_AVAILABLE == 2
        viz->visualize();
#endif
      }
