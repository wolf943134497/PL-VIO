...
struct StateOptions {
  ...
  ov_type::Line_Landmark_Rep::Line_Representation line_rep_msckf = ov_type::Line_Landmark_Rep::Line_Representation::CP_LINE;
  ...
  void print(const std::shared_ptr<ov_core::YamlParser> &parser = nullptr) {
    if (parser != nullptr) {
      ...
      std::string rep4 = ov_type::Line_Landmark_Rep::as_string(line_rep_msckf);
      parser->parse_config("line_rep_msckf", rep4);
      line_rep_msckf = ov_type::Line_Landmark_Rep::from_string(rep4);
    ...
    PRINT_DEBUG("  - line_rep_msckf: %s\n", ov_type::Line_Landmark_Rep::as_string(line_rep_msckf).c_str());
  }
};
