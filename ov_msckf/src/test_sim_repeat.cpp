...
int main(int argc, char **argv) {
  ...
  std::vector<std::vector<std::vector<std::pair<size_t, Eigen::VectorXf>>>> vec_lines;
  ...
  while (sim1.ok()) {
    ...
    std::vector<std::vector<std::pair<size_t, Eigen::VectorXf>>> lines;
    bool hascam = sim1.get_next_cam(time_cam, camids, feats, lines);
    if (hascam) {
      vec_camtime.push_back(time_cam);
      vec_feats.push_back(feats);
      vec_lines.push_back(lines);
    }
  }
  ...
  while (sim2.ok()) {
    ...
    std::vector<std::vector<std::pair<size_t, Eigen::VectorXf>>> feats;
    std::vector<std::vector<std::pair<size_t, Eigen::VectorXf>>> lines;
    bool hascam = sim2.get_next_cam(time_cam, camids, feats, lines);
    if (hascam) {
