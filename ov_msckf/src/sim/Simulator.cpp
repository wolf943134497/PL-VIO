...
Simulator::Simulator(VioManagerOptions &params_) {
  ...
  size_t linemapsize = linemap.size();
  ...
  for (int i = 0; i < params.state_options.num_cameras; i++) {
    ...
    while (true) {
      ...
      std::vector<std::pair<size_t, Eigen::VectorXf>> line_uvs = project_line(R_GtoI, p_IinG, i, linemap);
      ...
      if ((int)line_uvs.size() < params.num_lines) {
        generate_lines(R_GtoI, p_IinG, i, linemap, params.num_lines - (int)line_uvs.size());
      }
...
bool Simulator::get_next_cam(double &time_cam, std::vector<int> &camids, std::vector<std::vector<std::pair<size_t, Eigen::VectorXf>>> &feats, std::vector<std::vector<std::pair<size_t, Eigen::VectorXf>>> &lines) {
  ...
  for (int i = 0; i < params.state_options.num_cameras; i++) {
    ...
    if ((int)line_uvs.size() < params.num_lines) {
      PRINT_WARNING(YELLOW "[SIM]: cam %d was unable to generate enough lines (%d < %d projections)\n" RESET, (int)i, (int)line_uvs.size(), params.num_lines);
    }
    ...
    if ((int)line_uvs.size() > params.num_pts) {
      line_uvs.erase(line_uvs.begin() + params.num_pts, line_uvs.end());
    }
    ...
    for (size_t ff = 0; ff < line_uvs.size() && !params.use_stereo; ff++) {
      line_uvs.at(ff).first += i * linemap.size();
    }
    ...
    for (size_t jj = 0; jj < line_uvs.size(); jj++) {
      line_uvs.at(jj).second(0) += params.msckf_options.sigma_pix * w(gen_meas_cams.at(i))/2;
      line_uvs.at(jj).second(1) += params.msckf_options.sigma_pix * w(gen_meas_cams.at(i))/2;
      line_uvs.at(jj).second(2) += params.msckf_options.sigma_pix * w(gen_meas_cams.at(i))/2;
      line_uvs.at(jj).second(3) += params.msckf_options.sigma_pix * w(gen_meas_cams.at(i))/2;
    }
    

    // Push back for this camera
    feats.push_back(uvs);
    lines.push_back(line_uvs);
    camids.push_back(i);
  }
...
std::vector<std::pair<size_t, Eigen::VectorXf>> Simulator::
project_pointcloud(const Eigen::Matrix3d &R_GtoI, const Eigen::Vector3d &p_IinG, int camid, const std::unordered_map<size_t, Eigen::Vector3d> &feats) {
...
std::vector<std::pair<size_t, Eigen::VectorXf>> Simulator::project_line(const Eigen::Matrix3d &R_GtoI, const Eigen::Vector3d &p_IinG, int camid, const std::unordered_map<size_t, Eigen::Matrix<double, 6, 1>> &lines) {
  ...
  // Our projected uv true measurements
  std::vector<std::pair<size_t, Eigen::VectorXf>> lines_uv;

  // Loop through our map
  for (const auto &line : lines) {

    // Transform feature into current camera frame
    Eigen::Vector3d p_F1inI = R_GtoI * (line.second.block(0, 0, 3, 1) - p_IinG);
    Eigen::Vector3d p_F1inC = R_ItoC * p_F1inI + p_IinC;
    Eigen::Vector3d p_F2inI = R_GtoI * (line.second.block(3, 0, 3, 1) - p_IinG);
    Eigen::Vector3d p_F2inC = R_ItoC * p_F2inI + p_IinC;

    // Skip cloud if too far away
    if (p_F1inC(2) > params.sim_max_feature_gen_distance || p_F1inC(2) < 0.1)
      continue;
    
    if (p_F2inC(2) > params.sim_max_feature_gen_distance || p_F2inC(2) < 0.1)
      continue;

    // Project to normalized coordinates
    Eigen::Vector2f uv_norm1, uv_norm2;
    uv_norm1 << (float)(p_F1inC(0) / p_F1inC(2)), (float)(p_F1inC(1) / p_F1inC(2));
    uv_norm2 << (float)(p_F2inC(0) / p_F2inC(2)), (float)(p_F2inC(1) / p_F2inC(2));

    // Distort the normalized coordinates
    Eigen::Vector2f uv_dist1, uv_dist2;
    uv_dist1 = camera->distort_f(uv_norm1);
    uv_dist2 = camera->distort_f(uv_norm2);
    //uv_dist1 = uv_norm1;
    //uv_dist2 = uv_norm2;


    // Check that it is inside our bounds
    if (uv_dist1(0) < 0 || uv_dist1(0) > camera->w() || uv_dist1(1) < 0 || uv_dist1(1) > camera->h() 
       || uv_dist2(0) < 0 || uv_dist2(0) > camera->w() || uv_dist2(1) < 0 || uv_dist2(1) > camera->h()) {
      continue;
    }
    
    Eigen::Matrix<float,4,1> two_uv;
    two_uv << uv_dist1(0), uv_dist1(1), uv_dist2(0), uv_dist2(1);
    lines_uv.push_back({line.first, two_uv});
  }
...
void Simulator::generate_points(const Eigen::Matrix3d &R_GtoI, const Eigen::Vector3d &p_IinG, int camid,
  ...
  myfile1.open ("/home/zhangyanyu/catkin_ws_ov/src/open_vins/Debug/sim_feat_gt.txt", std::ofstream::app);
  ...
  for (int i = 0; i < numpts; i++) {
    ...
    featmap.insert({id_map1, p_FinG});
    id_map1++;
    
    myfile1 << "[Groundtruth] feature ID: " << id_map1 << " -> (" <<p_FinG(0)<<" "<<p_FinG(1)<<" "<<p_FinG(2)<<")\n";
...
void Simulator::generate_lines(const Eigen::Matrix3d &R_GtoI, const Eigen::Vector3d &p_IinG, int camid,
                                std::unordered_map<size_t, Eigen::Matrix<double, 6, 1>> &lines, int numlns) {

  std::ofstream myfile2;
  myfile2.open ("/home/zhangyanyu/catkin_ws_ov/src/open_vins/Debug/sim_line_gt.txt", std::ofstream::app);
  ...
  for (int i = 0; i < numlns; i++) {
    ...
    // Uniformly randomly generate within our fov
    std::uniform_real_distribution<double> gen_u1(camera->w()/4, camera->w()/2);
    std::uniform_real_distribution<double> gen_v1(0, camera->h());
    std::uniform_real_distribution<double> gen_u2(camera->w()/2, camera->w()*3/4);
    std::uniform_real_distribution<double> gen_v2(0, camera->h());
    double u_dist1 = gen_u1(gen_state_init);
    double v_dist1 = gen_v1(gen_state_init);
    double u_dist2 = gen_u2(gen_state_init);
    double v_dist2 = gen_v2(gen_state_init);

    // Convert to opencv format
    cv::Point2f uv_dist1((float)u_dist1, (float)v_dist1);
    cv::Point2f uv_dist2((float)u_dist2, (float)v_dist2);

    // Undistort this point to our normalized coordinates
    cv::Point2f uv_norm1, uv_norm2;
    uv_norm1 = camera->undistort_cv(uv_dist1);
    uv_norm2 = camera->undistort_cv(uv_dist2);
    //uv_norm1 = uv_dist1;
    //uv_norm2 = uv_dist2;

    // Generate a random depth
    std::uniform_real_distribution<double> gen_depth1(params.sim_min_feature_gen_distance, params.sim_max_feature_gen_distance);
    std::uniform_real_distribution<double> gen_depth2(params.sim_min_feature_gen_distance, params.sim_max_feature_gen_distance);
    double depth1 = gen_depth1(gen_state_init);
    double depth2 = gen_depth2(gen_state_init);

    // Get the 3d point
    Eigen::Vector3d bearing1, bearing2;
    bearing1 << uv_norm1.x, uv_norm1.y, 1;
    bearing2 << uv_norm2.x, uv_norm2.y, 1;
    Eigen::Vector3d p1_FinC, p2_FinC;
    p1_FinC = depth1 * bearing1;
    p2_FinC = depth2 * bearing2;

    // Move to the global frame of reference
    Eigen::Vector3d p1_FinI = R_ItoC.transpose() * (p1_FinC - p_IinC);
    Eigen::Vector3d p1_FinG = R_GtoI.transpose() * p1_FinI + p_IinG;
    Eigen::Vector3d p2_FinI = R_ItoC.transpose() * (p2_FinC - p_IinC);
    Eigen::Vector3d p2_FinG = R_GtoI.transpose() * p2_FinI + p_IinG;

    Eigen::Matrix<double, 6, 1> plk = endpoints_2_plk(p1_FinG, p2_FinG);
    Eigen::Matrix<double, 3, 1> n_e1_in_G = plk.block(0, 0, 3, 1)/plk.block(0, 0, 3, 1).norm();
    Eigen::Matrix<double, 3, 1> v_e1_in_G = plk.block(3, 0, 3, 1)/plk.block(3, 0, 3, 1).norm();

    Eigen::Matrix<double, 4, 1> quat = rot_2_quat(plk_2_rot(plk));
    double d = plk_2_d(plk);
    Eigen::Matrix<double, 4 ,1> cp = quat_2_cp(quat, d);

    myfile2 << "[Groundtruth] n_e1_in_G: " << id_map2 << " -> " << n_e1_in_G(0)<<" "<<n_e1_in_G(1)<<" "<<n_e1_in_G(2)<<"\n";
    myfile2 << "[Groundtruth] v_e1_in_G: " << id_map2 << " -> " <<v_e1_in_G(0)<<" "<<v_e1_in_G(1)<<" "<<v_e1_in_G(2)<<"\n";
    myfile2 << "[Groundtruth] CP-line: " << id_map2 << " -> (" <<cp(0)<<" "<<cp(1)<<" "<<cp(2)<<" "<<cp(3)<<")\n";
    myfile2 << "[Groundtruth] line ID: " << id_map2 << " -> (" <<quat(0)<<" "<<quat(1)<<" "<<quat(2)<<" "<<quat(3)<<")" << " line depth -> " << d << "\n";
    
    // Append this as a new feature
    Eigen::Matrix<double,6,1> two_endpoint;
    two_endpoint << p1_FinG(0), p1_FinG(1), p1_FinG(2), p2_FinG(0), p2_FinG(1), p2_FinG(2);
    linemap.insert({id_map2, two_endpoint});
    id_map2++;
  }
  myfile2.close();
}
