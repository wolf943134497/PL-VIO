...
public:
  ...
  std::unordered_map<size_t, Eigen::Matrix<double, 6, 1>> get_linemap() { return linemap; }
  ...
  std::vector<Eigen::Matrix<double, 6, 1>> get_linemap_vec() {
    std::vector<Eigen::Matrix<double, 6, 1>> lines;
    for (auto const &line : linemap)
