...
VioManager::VioManager(VioManagerOptions &params_) : thread_init_running(false), thread_init_success(false) {
  ...
  trackLineDATABASE = std::make_shared<LineDatabase>();
  ...
  if (params.use_klt) {
    ...
    trackLINES = std::shared_ptr<LineTrackBase>(new TrackLineDescriptor(
        state->_cam_intrinsics_cameras, params.init_options.init_max_lines, state->_options.max_aruco_features, params.use_stereo,
        params.line_histogram_method));
  ...
  initializer = std::make_shared<ov_init::InertialInitializer>(params.init_options, trackFEATS->get_feature_database(), trackLINES->get_line_database());


  // Make the updater!
  updaterMSCKF = std::make_shared<UpdaterMSCKF>(params.msckf_options, params.featinit_options, params.lineinit_options, params.use_line);
  updaterSLAM = std::make_shared<UpdaterSLAM>(params.slam_options, params.aruco_options, params.featinit_options);

  // If we are using zero velocity updates, then create the updater
  if (params.try_zupt) {
    updaterZUPT = std::make_shared<UpdaterZeroVelocity>(params.zupt_options, params.imu_noises, trackFEATS->get_feature_database(), trackLINES->get_line_database(), propagator, params.gravity_mag, params.zupt_max_velocity, params.zupt_noise_multiplier, params.zupt_max_disparity);
  }
  ...
  active_line_tracks_initializer = std::make_shared<LineInitializer>(params.lineinit_options);
}


void VioManager::feed_measurement_simulation(double timestamp, const std::vector<int> &camids,
                                             const std::vector<std::vector<std::pair<size_t, Eigen::VectorXf>>> &feats, const std::vector<std::vector<std::pair<size_t, Eigen::VectorXf>>> &lines) {
  ...
  std::shared_ptr<LineTrackSIM> linetrackSIM = std::dynamic_pointer_cast<LineTrackSIM>(trackLINES);
  ...
  if (trackSIM == nullptr) {
    ...
    linetrackSIM = std::make_shared<LineTrackSIM>(state->_cam_intrinsics_cameras, state->_options.max_aruco_features);
    trackLINES = linetrackSIM;
    PRINT_WARNING(RED "[SIM]: casting our tracker to a TrackSIM object!\n" RESET);
  }

  // Feed our simulation tracker
  trackSIM->feed_measurement_simulation(timestamp, camids, feats);
  linetrackSIM->feed_measurement_simulation(timestamp, camids, lines);

  if (is_initialized_vio) {
    trackDATABASE->append_new_measurements(trackSIM->get_feature_database());
    trackLineDATABASE->append_new_measurements(linetrackSIM->get_line_database());
  }
...
void VioManager::track_image_and_update(const ov_core::CameraData &message_const) {
  ...
  trackLINES->feed_new_camera(message);
  
  if (is_initialized_vio) {
    trackDATABASE->append_new_measurements(trackFEATS->get_feature_database());
    trackLineDATABASE->append_new_measurements(trackLINES->get_line_database());
  }
...
void VioManager::do_feature_propagate_update(const ov_core::CameraData &message) {
  ...
  std::vector<std::shared_ptr<Line>> lines_lost, lines_marg, lines_slam;
  
  feats_lost = trackFEATS->get_feature_database()->features_not_containing_newer(state->_timestamp, false, true);
  lines_lost = trackLINES->get_line_database()->lines_not_containing_newer(state->_timestamp, false, true);
  ...
  if ((int)state->_clones_IMU.size() > state->_options.max_clone_size) {
    ...
    lines_marg = trackLINES->get_line_database()->lines_containing(state->margtimestep(), false, true);
  ...
  auto it11 = lines_lost.begin();
  while (it11 != lines_lost.end()) {
    bool found_current_message_camid = false;
    for (const auto &camuvpair : (*it11)->startpoint) {
      if (std::find(message.sensor_ids.begin(), message.sensor_ids.end(), camuvpair.first) != message.sensor_ids.end()) {
        found_current_message_camid = true;
        break;
      }
  ...
  it11 = lines_lost.begin();
  while (it11 != lines_lost.end()) {
    if (std::find(lines_marg.begin(), lines_marg.end(), (*it11)) != lines_marg.end()) {
      // PRINT_WARNING(YELLOW "FOUND FEATURE THAT WAS IN BOTH feats_lost and feats_marg!!!!!!\n" RESET);
      it11 = lines_lost.erase(it11);
    } else {
      it11++;
  ...
  std::vector<std::shared_ptr<Line>> lines_maxtracks;
  auto it22 = lines_marg.begin();
  while (it22 != lines_marg.end()) {
    // See if any of our camera's reached max track
    bool reached_max = false;
    for (const auto &cams : (*it22)->timestamps) {
      if ((int)cams.second.size() > state->_options.max_clone_size) {
        reached_max = true;
        break;
      }
    ...
    // If max track, then add it to our possible slam feature list
    if (reached_max) {
      lines_maxtracks.push_back(*it22);
      it22 = lines_marg.erase(it22);
    } else {
      it22++;
    }
  }
  ...
  std::vector<std::shared_ptr<Line>> linesup_MSCKF = lines_lost;
  
  featsup_MSCKF.insert(featsup_MSCKF.end(), feats_marg.begin(), feats_marg.end());
  featsup_MSCKF.insert(featsup_MSCKF.end(), feats_maxtracks.begin(), feats_maxtracks.end());
  
  linesup_MSCKF.insert(linesup_MSCKF.end(), lines_marg.begin(), lines_marg.end());
  linesup_MSCKF.insert(linesup_MSCKF.end(), lines_maxtracks.begin(), lines_maxtracks.end());
  ...
  std::sort(linesup_MSCKF.begin(), linesup_MSCKF.end(), [](const std::shared_ptr<Line> &a, const std::shared_ptr<Line> &b) -> bool {
    size_t asize = 0;
    size_t bsize = 0;
    for (const auto &pair : a->timestamps)
      asize += pair.second.size();
  ...
  if ((int)linesup_MSCKF.size() > state->_options.max_msckf_in_update)
    linesup_MSCKF.erase(linesup_MSCKF.begin(), linesup_MSCKF.end() - state->_options.max_msckf_in_update);
  //===================================================================================

  updaterMSCKF->update(state, featsup_MSCKF, linesup_MSCKF);
  ...
  for (auto const &line : linesup_MSCKF) {
    //Eigen::Matrix<double, 6, 1> good_line;
    //good_line << line->startpoint, line->endpoint;
    //good_lines_MSCKF.push_back(good_line);
    good_lines_MSCKF.push_back(line->p_LinG);
    line->to_delete = true;
  }
  ...
  trackLINES->get_line_database()->cleanup();
  ...
  if ((int)state->_clones_IMU.size() > state->_options.max_clone_size) {
    ...
    trackLINES->get_line_database()->cleanup_measurements(state->margtimestep());
    trackLineDATABASE->cleanup_measurements(state->margtimestep());
  ...
  PRINT_DEBUG(BLUE "[TIME]: %.4f seconds for MSCKF update (%d feats) (%d lines)\n" RESET, time_msckf, (int)featsup_MSCKF.size(), (int)linesup_MSCKF.size());
  if (state->_options.max_slam_features > 0) {
...
bool VioManager::try_to_initialize(const ov_core::CameraData &message) {
  ...
  std::thread thread([&] {
    ...
    if (success) {
      ...
      trackLINES->get_line_database()->cleanup_measurements(state->_timestamp);
