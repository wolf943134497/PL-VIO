...
Simulator::Simulator(InertialInitializerOptions &params_) {
  ...
  for (int i = 0; i < params.num_cameras; i++) {
    ...
    while (true) {
      ...
      if ((int)line_uvs.size() < params.init_max_features) {
        generate_lines(R_GtoI, p_IinG, i, linemap, params.init_max_lines - (int)line_uvs.size());
      }
...
bool Simulator::get_next_cam(double &time_cam, std::vector<int> &camids,
                             std::vector<std::vector<std::pair<size_t, Eigen::VectorXf>>> &feats, std::vector<std::vector<std::pair<size_t, Eigen::VectorXf>>> &lines) {
  ...
  for (int i = 0; i < params.num_cameras; i++) {
    ...
    if ((int)line_uvs.size() > params.init_max_features) {
      line_uvs.erase(line_uvs.begin() + params.init_max_features, line_uvs.end());
    }
    ...
    for (size_t jj = 0; jj < line_uvs.size(); jj++) {
      line_uvs.at(jj).second(0) += params.sigma_pix * w(gen_meas_cams.at(i))/2;
      line_uvs.at(jj).second(1) += params.sigma_pix * w(gen_meas_cams.at(i))/2;
      line_uvs.at(jj).second(2) += params.sigma_pix * w(gen_meas_cams.at(i))/2;
      line_uvs.at(jj).second(3) += params.sigma_pix * w(gen_meas_cams.at(i))/2;
    }
...
std::vector<std::pair<size_t, Eigen::VectorXf>> Simulator::project_pointcloud(const Eigen::Matrix3d &R_GtoI, const Eigen::Vector3d &p_IinG, int camid, const std::unordered_map<size_t, Eigen::Vector3d> &feats) {
...
void Simulator::generate_lines(const Eigen::Matrix3d &R_GtoI, const Eigen::Vector3d &p_IinG, int camid,
  ...
  for (int i = 0; i < numlns; i++) {
    ...
    // Eigen::Matrix3d rot = plk_2_rot(plk);
    // Eigen::Matrix<double, 4, 1> quat = rot_2_quat(rot);
    // Eigen::Matrix<double,4,1> cp = quat_2_cp(quat, (skew_x(p1_FinG)*p2_FinG).norm());
